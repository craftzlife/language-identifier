...
- **v4.1 status**: Layer 9's default impl `FastTextClassifier` ships behind the `ml-fasttext` cargo feature, backed by Meta/FAIR's `lid.176.bin` (pure-Rust `fasttext` crate v0.8 — no C++ toolchain needed). The classifier intentionally drops fastText's `zh` label so Hans/Hant disambiguation stays in Layer 3. Layer 10 remains scaffolding-only; default impl lands in v4.2.

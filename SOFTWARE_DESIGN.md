...
- **v4.2 status**: Layer 9's default impl `OpenLidClassifier` ships behind the `ml-openlid` cargo feature, backed by the OpenLID v3 fastText model (`openlid-v3.bin`, 194 languages, `iso639-3_Script` labels — pure-Rust `fasttext` crate v0.8, no C++ toolchain needed). OpenLID's `cmn_Hans` / `cmn_Hant` labels are forwarded to `bcp47::resolve_zh` so the Sinitic Tier-2 router can promote them to `yue` / `lzh` / `zh-Hant-HK` / `zh-Hant-TW` when text evidence justifies it; `yue_Hant` / `lzh_Hani` map straight through. The supported tag set is split into two tiers: 14 Tier 1 tags get the full multi-layer pipeline (scripts, lexicons, morphology, orthography markers); the remaining ~180 Tier 2 tags are ML-only and surface as candidates when OpenLID is confident. Tier 2 inputs may report `Status::Ambiguous` because the L9 bonus is capped at 0.30 (see `aggregate.rs`); follow-up work can tune calibration if Tier 2 resolution becomes load-bearing.
